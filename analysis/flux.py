...
flux0 = (chan0/(chan0eff/100))/(720*1594)
flux0err = np.sqrt((((((1/(chan0eff/100))/(720*1594))*np.sqrt(chan0))**2))+ (((-(chan0/(chan0eff)**2)/(720*1594))*err0)**2))
flux1 = (chan1/(chan1eff/100))/(720*1594)
flux1err = np.sqrt((((((1/(chan1eff/100))/(720*1594))*np.sqrt(chan1))**2))+ (((-(chan1/(chan1eff)**2)/(720*1594))*err1)**2))
flux2 = (chan2/(chan2eff/100))/(720*1594)
flux2err = np.sqrt((((((1/(chan2eff/100))/(720*1594))*np.sqrt(chan2))**2))+ (((-(chan2/(chan2eff)**2)/(720*1594))*err2)**2))
flux3 = (chan3/(chan2eff/100))/(720*1594)
flux3err = np.sqrt((((((1/(chan3eff/100))/(720*1594))*np.sqrt(chan3))**2))+ (((-(chan3/(chan3eff)**2)/(720*1594))*err3)**2))
print(u"Flux across Channel 0: {} \u00B1 {} events per min per cm\u00b2".format(flux0,flux0err))
print(u"Flux across Channel 1: {} \u00B1 {} events per min per cm\u00b2".format(flux1,flux1err))
print(u"Flux across Channel 2: {} \u00B1 {} events per min per cm\u00b2".format(flux2,flux2err))
print(u"Flux across Channel 3: {} \u00B1 {} events per min per cm\u00b2".format(flux3,flux3err))
